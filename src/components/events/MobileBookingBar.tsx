...
export default function MobileBookingBar({
  ...
  // Observe inline CTA to toggle fixed bar
  useEffect(() => {
  ...
  const hidden = inlineVisible || keyboardOpen;

  const fixedBarClasses = cn(
    "md:hidden fixed inset-x-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-800 px-4 py-3 transition-all duration-300",
    hidden && "translate-y-full opacity-0 pointer-events-none",
    navHidden || keyboardOpen ? "bottom-0 safe-area-bottom" : "bottom-16",
  );

  // Determine CTA label and action for each state
  ...
  return (
    <>
      {/* ── Fixed bar (hides when inline section is visible) ── */}
      ...
      {/* ── Inline CTA (Eventbrite-style pill, scrolls with page) ── */}
      <div ref={inlineRef} className="md:hidden px-4 pb-8 pt-4">
