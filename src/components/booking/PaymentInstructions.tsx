...
export default function PaymentInstructions({
  ...
  const qrUrl = method === "gcash" ? paymentInfo.gcash_qr_url : paymentInfo.maya_qr_url;
    ...
    return (
      <div className="bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4">
        <p className="text-sm text-yellow-700 dark:text-yellow-300">
          The club hasn&apos;t set up their {label} details yet. Please try another payment method.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`${wrapperClass} rounded-xl p-4 space-y-3`}>
      ...
      {qrUrl && (
        <div className="bg-white dark:bg-gray-900 rounded-lg p-3 flex justify-center">
          <Image
            src={qrUrl}
            alt={`${label} QR Code`}
            width={200}
      ...
      {number && (
        <div className="bg-white dark:bg-gray-900 rounded-lg p-3 text-center">
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
            {qrUrl ? "Or send to this number" : `Send to this ${label} number`}
          </p>
      ...
      <ol className="text-sm text-gray-600 dark:text-gray-400 space-y-1 list-decimal list-inside">
        ...
        <li>
          {qrUrl ? "Scan the QR code or send" : "Send"}
          {amount ? ` ₱${amount.toLocaleString()}` : " the exact amount"}
          {number && !qrUrl ? " to the number above" : ""}
        </li>
